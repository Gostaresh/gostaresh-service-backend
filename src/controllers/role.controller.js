...
exports.update = async (req, res, next) => {
  try {
    const { error: p } = idParam.validate(req.params);
    if (p) return res.status(400).json({ message: p.message });
...
exports.setPermissions = async (req, res, next) => {
  try {
    const { error: p } = idParam.validate(req.params);
    if (p) return res.status(400).json({ message: p.message });
    const { error, value } = permsSchema.validate(req.body);
    if (error) return res.status(400).json({ message: error.message });
    const out = await service.setPermissions(
      req.params.id,
      value.permissionIDs
    );
    res.json(out);
...
exports.setUserRoles = async (req, res, next) => {
  try {
    ...
    const { error: p } = schema.validate(req.params);
    if (p) return res.status(400).json({ message: p.message });
    const { error, value } = userRolesSchema.validate(req.body);
    if (error) return res.status(400).json({ message: error.message });
    await service.setUserRoles(req.params.id, value.roleIDs);
