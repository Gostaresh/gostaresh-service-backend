...
router.post("/upload", upload.single("file"), async (req, res, next) => {
  try {
    ...
    const rawScope = req.body.scope || req.query.scope || "misc";
    const safeScope = sanitizePathSegments(rawScope);
    const relPath = ["/uploads", safeScope, req.file.filename].filter(Boolean).join("/").replace(/\\/g, "/");
...
router.post("/upload/multi", upload.array("files"), async (req, res, next) => {
  try {
    const rawScope = req.body.scope || req.query.scope || "misc";
    const safeScope = sanitizePathSegments(rawScope);
    const files = (req.files || []).map((f) => ({
      path: ["/uploads", safeScope, f.filename].filter(Boolean).join("/").replace(/\\/g, "/"),
      fileName: f.filename,
...
router.delete("/delete", async (req, res, next) => {
  try {
    ...
    if (filePath) {
      ...
      if (!scope || !fileName) {
        return res.status(400).json({ message: "Provide either 'path' or both 'scope' and 'fileName'" });
      }
    ...
    return res.json({ message: "Deleted", path: (req.body?.path || req.query?.path) || null });
  } catch (err) {
    next(err);
  }
});
...
router.delete("/delete-scope", async (req, res, next) => {
  try {
    const scope = req.body?.scope || req.query?.scope;
    if (!scope) {
    ...
    if (path.resolve(dirAbs) === path.resolve(getUploadsRoot())) {
      return res.status(400).json({ message: "Refusing to delete uploads root" });
    }
